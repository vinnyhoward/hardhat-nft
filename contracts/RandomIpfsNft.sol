...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    // When minting NFT we will trigger a Chainlink VRF call
    // to receive our random number so that we can randomize
    // the NFT minted
    ...
    function _initializeContract(string[3] memory dogTokenUris) private {
        if (s_initialized) {
            revert RandomDoggyDog__AlreadyInitialized();
        }
        s_dogTokenUris = dogTokenUris;
        s_initialized = true;
    }
    ...
    function getBreedFromModdedRng(uint256 moddedRng) public pure returns (Breed) {
        ...
        for (uint256 i = 0; i < chanceArray.length; i++) {
            // Pug = 0 - 9  (10%)
            // Shiba-inu = 10 - 39  (30%)
            // St. Bernard = 40 = 99 (60%)s
            if (moddedRng >= cumulativeSum && moddedRng < chanceArray[i]) {
                return Breed(i);
            }
            cumulativeSum = chanceArray[i];
        }
        revert RandomDoggyDog__RangeOutOfBounds();
    }
    ...
    function withdraw() public onlyOwner {
        ...
        if (!success) {
            revert RandomDoggyDog__TransferFailed();
        }
    }
